...
for t in "${@:-tests/lockout.mjs tests/pages.mjs tests/dashboard.mjs tests/taskfields.mjs tests/csv.mjs tests/webhooks.mjs tests/history.mjs tests/idempotency.mjs tests/cursor.mjs tests/shortcuts.mjs tests/mobile.mjs tests/uploads.mjs tests/live.mjs tests/a11y.mjs tests/devices.mjs tests/session.mjs}"; do
  for f in $t; do
    printf '\n\033[1;34m==> %s\033[0m\n' "$f"
    node "$f" || failed=1
  done
